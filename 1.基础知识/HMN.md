...
在优化问题中 $f_0,\,f_1,\dots,\,f_m$ 都是凸函数时，就是一个凸优化问题。与线性规划类似，同样通常使用内点法求解，它每一步迭代的时间复杂度为
$$
\max\{n^3,n^2m,F\},
$$
...
*  $K=\{c\in R^n|c_1+c_2t+\dots+c_nt^{n-1}\ge0\ \mathrm{for}\ t\in[0,1]\},$ 是多项式非负真锥。
...
> $y\in K^*\quad\Leftrightarrow \quad (y^T)(x)\ge0\ \ \mathrm{for all}\ x\in K$
...
**定义**：epigraph 就是函数的上界面，
$$
\mathrm{epi}\ f=\left\{(x,t)\,|\,x\in \mathrm{dom}\ f,\ f(x)\le t \right\}.
$$
**定理**：函数 $f(x)$ 是凸函数 $\Leftrightarrow$ $\mathrm{epi}\ f$ 是凸集

**用途**：epigraph 将函数转化为集合，从集合的角度证明凸函数更容易。
...
**定义**：共轭函数 $f^*(y)$ 表示原函数 $f(x)$ 与过原点平面 $x^Ty$ 的最大距离（寻找 $x$ 使之成立），数学表达式为
$$
...
**定义**：若函数 $f:R^n\rightarrow R$ 的所有 sublevel sets $S_\alpha=\left\{ x\in dom\,f\,|\,f(x)\le\alpha \right\}$ 都是凸的，那么函数 $f$ 是一个拟凸函数。如图所示，
...
这个定理衍生出了以下两个**推论**：
  ...
  * 向量中的非零元素的个数：$\mathrm{card}(x+y)\ge \min\left\{\mathrm{card}(x),\mathrm{card}(y)\right\};\ x,y\ge0$；
* 半正定矩阵的秩：$\mathrm{rank}(X+Y)\ge \min\left\{\mathrm{rank}\,X,\mathrm{rank}\,Y\right\};\ X,Y\in\mathbb{S}_+^n$；
  
* **单调性**：$f$ 是拟凸的 $\Leftrightarrow$ 

  * $f$ 是单调的；
  * 存在点 $c\in dom\,f$，当 $t\le c$ 时是非增的，当 $t\ge c$ 时是非减的。
...
**定义**：若在 $x$ 的定义域内，有 $f(x)>0$，使 $\log f$ 是凹的，那么称 $f$ 是**对数凹函数**，$1/f$ 是**对数凸函数**

进一步地，可以将条件 "$\log f$ 是凹的" 解释为：对于 $x,y\in dom\,f$ 有
$$
...
***proof***： 上式两边取 $\log$ 可以写成
$$
\log{f(\theta x+(1-\theta)y)}\ge \theta\log f(x)+(1-\theta)\log f(y).
$$
若令 $a_1,a_2=\theta x+(1-\theta)y$，在 $f$ 函数上的取值如左图所示。再以 $f$ 作为 $\log f$ 的定义域，令 $f(b)=\theta\log f(x)+(1-\theta)\log f(y)$，得到各点在 $log f$ 上的取值，如右图所示。

<center class="half">
    <img src="./img-ch1/h-logconcave1.png" width="400"/><img src="./img-ch1/h-logconcave2.png" width="400"/>
</center>
...
* 非负的凹函数一定是对数凹函数
* 对数凹（凸）函数是拟凹（凸）函数，（由于对数函数是单增的）

**例子**：

* 仿射方程 $f(x)=a^Tx+b$ 在 $\{x\,|\,a^Tx+b>0\}$ 上是对数凹函数；

* 幂函数 $f(x)=x^a$ 在 $a\ge 0$ 时是对数凹的，$a\le0$ 时是对数凸函数；

* 指数函数 $f(x)=e^{ax}$ 是对数凹凸函数；
...
* Gamma函数在 $x\ge1$ 上是对数凹函数；
  $$
  \Gamma(x)=\int_0^\infty u^{x-1}e^{-u}du
  $$

* 正定矩阵的行列式是对数凹的；
* 正定矩阵的行列式除以矩阵的迹 $\det(X)/\mathrm{tr}(X)$ 是对数凹函数；
* 相当多的概率密度矩阵是对数凹函数，如多维正态分布和指数分布；



### 4.6.1. 对数凹函数的判别方法

* **二阶微分**：对数函数 $\log f(x)$ 的二阶微分为
  $$
  \nabla^2\log f(x)=\frac{1}{f(x)}\nabla^2f(x)-\frac{1}{f(x)^2}\nabla f(x)\nabla f(x)^T.
  $$
  因此有：

  * $f$ 是对数凸函数 $\Leftrightarrow$ $\nabla^2\log f(x)\ge 0\Rightarrow f(x)\nabla^2 f(x)\ge\nabla f(x)\nabla f(x)^T$

  * $f$ 是对数凹函数 $\Leftrightarrow$ $\nabla^2\log f(x)\le 0\Rightarrow f(x)\nabla^2 f(x)\le\nabla f(x)\nabla f(x)^T$


### 4.6.2. 保持对数凹（凸）性的方法：

* **乘法** $h(x)=f(x)g(x)$ 不改变函数的对数凹（凸）性；

* **加法** $h(x)=f(x)+g(x)$ 可能会改变函数的对数凹性，但不会改变函数的对数凸性；

* **积分** $g(x)=\int_C f(x,y)dy$ 是一种特殊的加法，因此不会改变函数的对数凸性，但只有在特殊的函数如 $f:R^n\times R^m\rightarrow R$ 可以保持对数凹性，这个结论可以得出以下推论：

  * **卷积**：
    $$
    (f*g)(x)=\int f(x-y)g(y)\,dy
    $$
     其中 $f$ 和 $g$ 是两个对数凹函数，则卷积结果可以保持对数凹性；

  * **概率密度函数**：
    $$
    f(x)=\mathbf{prob}(\{x+w\}\in C)
    $$
    其中 $C\subseteq\mathrm{R}^n$ 是凸集，$w\in C$  是服从概率密度函数 $p$ （对数凹函数）的随机变量；可得 $f(x)=\int I_s(x+w)p(w)\,dw$，其中 $I_s$ 是凸集 $C$ 的指示函数；后面这个式子是保持对数凹性的；

  * **多元分布函数**：
    $$
    F(x)=\mathrm{prob}(x\le x)=\int_{-\infty}^{x_n}\cdots\int_{\infty}^{x_n}f(z)\,dz_1\cdots dz_n
    $$
    其中 $w$  是服从概率密度函数 $f$ （对数凹函数）的随机变量，那么 $F(x)$ 是一个对数凹函数；

  * 多角体 $P_u=\{x\in\mathrm{R}^n\,|\,Ax\le u\}$ 的体积是一个关于 $u$ 的对数凹函数。

## 4.7. 广义不等式下的凸函数判定方法

对于真锥 $K\subseteq \mathrm{R}^n$ 意义下的不等式 $\le_K$. 

### 4.7.1. 单调性

**定义**：

1）**向量单调** 函数：对于 $f:\mathrm{R}^n\rightarrow\mathrm{R}$ 有 $x_1\le y_1,\dots,x_n\le y_n\Rightarrow f(x)\le f(y)$；

2）**矩阵单调** 函数：函数 $f:\mathrm{S}^n\rightarrow \mathrm{R}$ 在半正定锥上单调；举例说明：

* 函数 $\mathrm{tr}(WX)$ 在 $W> 0$ 时单增，反之单减；

* 函数 $\mathrm{tr}(X^{-1})$ 在 $X\in \mathrm{S}^n_{++}$ 上单减；
* 函数 $\det(X)$ 在 $X\in \mathrm{S}^n_{++}$ 上单增，在 $X\in \mathrm{S}^n_{+}$ 上非减；

**梯度条件**：可微函数 $f$ 是非减的当且仅当 $\nabla f(x)\ge_{K^*}0$；

**不同之处**：严格条件下，当 $\nabla f(x)>_{K^*}0$，$f$ 是 $K$ - 单增的，但是反之不成立；

### 4.7.2. 凸判定

**向量凸函数判定方法**：

1）函数 $f:\mathrm{R}^n\rightarrow\mathrm{R}^m$ 是 $K$ - 凸的当且仅当
$$
f(\theta x+(1-\theta)y)\le_K \theta f(x)+(1-\theta)f(y).
$$
2）函数 $f:\mathrm{R}^n\rightarrow\mathrm{R}^m$ 是凸的当且仅当每个分函数 $f_i$ 都是凸的；

**矩阵凸函数判定方法**：

1）函数 $f:\mathrm{R}^n\rightarrow\mathrm{S}^m$ 是 $K$ - 凸的当且仅当
$$
f(\theta x+(1-\theta)y)\le \theta f(x)+(1-\theta)f(y).
$$
2）函数 $f:\mathrm{R}^n\rightarrow\mathrm{S}^m$ 是凸的当且仅当 $z^Tf(x)z$ 对于任意的 $z$ 都是凸的；

* 函数 $f(X)=XX^T$ 是矩阵凸函数；
* 函数 $f(X)=X^p$（$1\le p\le2$ 或 $-1\le p\le0$） $X\in\mathrm{S}^n_{++}$ 上是凸的；
* 函数 $f(X)=e^X$ 在 $\mathrm{S}^n,\,n\ge2$ 上是非凸的；

**$K$ - 凸判定：**

1）**对偶判定法**：函数 $f$ 是 $K$ - 凸的当且仅当：对于任意的 $w\ge_{K^*}0$，有 $w^Tf$ 是凸的；

2）**微分判定法**：函数 $f$ 是 $K$ - 凸的当且仅当：$f$ 的 $\mathrm{domain}$ 是凸的且
$$
f(y)\ge_K f(x)+\frac{\partial f(x)}{\partial x}(y-x).
$$
**分解定理**：没看懂？
